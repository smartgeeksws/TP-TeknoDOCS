        ...
        source = {
            ...
            "properties": {
                ...
                "apa_reference": {
                    "type": "string",
                    "pattern": r"^[\s\S]+$",
                    "description": "Referencia bibliográfica completa en formato APA 7. No incluyas DOI, URL ni fecha de recuperación; el sistema agrega el localizador correcto.",
                },
            },
            "required": [
...
Cada afirmación técnica que lo requiera debe contener una cita en texto APA 7, consistente con las fuentes consultadas. Usa cita parentética con autor y año, por ejemplo (García, 2022), y separa varias fuentes con punto y coma, por ejemplo (García, 2022; Organización Mundial de la Salud, 2021). Para tres o más autores usa el primer autor seguido de et al. desde la primera cita. No inventes autores, años ni citas. La cobertura bibliográfica es obligatoria: cada fuente citada en cualquier campo narrativo y en glossary[].citation, similar_products[].citation, scientific_references[].citation o regulations[].citation debe tener una entrada única y correspondiente en sources, aunque la cita aparezca únicamente dentro de una tabla. No omitas fuentes citadas ni agregues referencias sin respaldo verificable. Completa sources[].apa_reference en formato APA 7, con autor corporativo o individual, fecha, título y datos específicos del tipo de fuente; usa título en la posición del autor cuando no exista autor. Ordena references alfabéticamente y asegúrate de que contenga todas las fuentes citadas. No incluyas DOI, URL ni fecha de recuperación en apa_reference; el sistema agrega un DOI o una URL directa como localizador. En campos narrativos y campos citation no incluyas enlaces Markdown, URL, dominios, parámetros UTM ni marcadores internos de herramientas de búsqueda; registra el DOI o la URL canónica únicamente en sources[].doi o sources[].url. Para una página institucional sin fecha usa {document_date.isoformat()} como fecha de consulta solo como metadato de auditoría, no como fecha de recuperación en la referencia. Selecciona solo normas aplicables. El cronograma debe adaptarse al tipo real de proyecto y contener fases, actividades y entregables, sin fechas. Si glossary_requested es falso, devuelve glossary vacío; si es verdadero, define exclusivamente los términos solicitados. No incluyas datos personales que no sean necesarios ni los inventes. El usuario realizará la revisión final del contenido y las fuentes.
""".strip()
